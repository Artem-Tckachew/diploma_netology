...
resource "yandex_vpc_route_table" "inner-to-nat" {
  ...
  static_route {
    ...
    next_hop_address   = yandex_compute_instance.bastionserv.network_interface.0.ip_address
  }
}
#----------------- subnet -----------------------------
resource "yandex_vpc_subnet" "subnet_webserv-1" {
  name           = "subnet_webserv-1"
  zone           = var.zone_a
  network_id     = yandex_vpc_network.dipnet.id
...
resource "yandex_vpc_subnet" "subnet_webserv-2" {
  name           = "subnet_webserv-2"
  zone           = var.zone_b
  network_id     = yandex_vpc_network.dipnet.id
